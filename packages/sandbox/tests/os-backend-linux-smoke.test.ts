...
describe('OCTOPUS_REQUIRE_OS_SANDBOX contract (os-backend)', () => {
  it('hard-fails when REQUIRE_OS=1 but the platform cannot run the real os-backend smoke', () => {
    if (REQUIRE_OS && !isLinux) {
      throw new Error(
        'OCTOPUS_REQUIRE_OS_SANDBOX=1 but platform is not linux — the os-backend smoke ' +
...
describe.skipIf(!isLinux)('OsSandboxBackend — real Linux smoke', () => {
