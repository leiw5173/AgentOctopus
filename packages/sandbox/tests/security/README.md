...
The macOS restricted lane is authored against a fixed filename (`macos-restricted-lane.test.ts`) that is referenced-but-unlanded until the supplemental macOS restricted backend (Task #18) completes. That job carries a guard step that fails loudly with a "lane not yet landed" message while the file is absent, instead of an opaque Vitest "no test files found" failure; the workflow remains a mandatory gate throughout. The privileged Linux lane (`linux-lane-setup.ts`, `linux-lane.test.ts`, `linux-topology.test.ts`) has landed and runs for real; its guard step now verifies the files are present rather than failing on absence.
