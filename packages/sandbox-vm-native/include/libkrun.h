/* ---------------------------------------------------------------------
 * VENDORED PIN — libkrun v1.19.4 public API (upstream include/libkrun.h)
 *
 * This file is the upstream containers/libkrun header at git tag v1.19.4,
 * vendored VERBATIM (fetched from raw.githubusercontent.com; the original
 * file's sha256 is recorded in the Task 11 report). The libkrun ABI used
 * by this package is PINNED to v1.19.4: every krun_* signature the helper
 * relies on was verified against this exact header revision.
 *
 * Task 15 (full vendoring) MAY replace this file with the upstream header
 * shipped alongside the vendored dylibs from the release artifact; the ABI
 * contract must remain v1.19.4.
 *
 * Only a subset of the declarations below is used by src/vm-helper.c:
 *   krun_create_ctx, krun_disable_implicit_vsock, krun_add_vsock,
 *   krun_add_vsock_port, krun_add_disk, krun_set_root_disk_remount,
 *   krun_set_vm_config, krun_add_virtio_console_multiport,
 *   krun_add_console_port_inout, krun_set_exec, krun_set_workdir,
 *   krun_start_enter, krun_has_feature (+ KRUN_FEATURE_BLK and the
 *   KRUN_TSI_HIJACK_* constants). This project calls NO virtio-net /
 *   passt / gvproxy API; those declarations ship here only because this
 *   is the complete upstream header.
 * --------------------------------------------------------------------- */
#ifndef _LIBKRUN_H
#define _LIBKRUN_H
