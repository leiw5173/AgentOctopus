 ...
 *
 * Moved from apps/cli to packages/skills so the install pipeline is
 * available to all consumers (CLI, web, gateway, agent-protocol).
 *
...
/**
 * Parse ZIP entries from a buffer using only Node built-ins.
 ...
 *
 * Creates:
 *   <targetDir>/<slug>/SKILL.md
 *   <targetDir>/<slug>/_meta.json
 *   <targetDir>/<slug>/scripts/*  (all scripts from index)
 *   <targetDir>/<slug>/**         (all other files: reference docs, data, configs)
 *
...
export function installFromIndex(
  ...
  const skillDir = path.join(targetDir, entry.slug);
  const scriptsDir = path.join(skillDir, 'scripts');
