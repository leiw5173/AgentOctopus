...
pnpm test             # run all tests across all workspaces (1,312 cases on macOS: 1,215 pass, 97 platform-gated skip; 139 in the security suite)
...
**Security-gate skip policy.** `security-gate` directly requires `produce-linux-artifacts` and `vm-hvf-probe` success; the producer runs on fork PRs too. Only a fork PR may skip `privileged-linux`, and `vm-lane` may skip only when a successful HVF probe explicitly reports `unavailable`. Dependency-induced skips are failures even if the skipped lanes otherwise look like approved exceptions. `tests/security/security-gate-workflow.test.mjs` executes the workflow shell gate against these cases.
