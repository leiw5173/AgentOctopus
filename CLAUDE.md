...
| VM (libkrun) | `vm-lane` (`macos-15`, `OCTOPUS_VM_LANE=1`) | Real libkrun guest L3/L4 escape matrix + G1/G2 qualification gates → gate manifest + signed outer release manifest. `security:build-vm` produces rootfs (mke2fs) + libkrun (source-build) + libkrunfw (prebuilt) + helper; `security:probe-vm` runs G1/G2. | SkipIf-gated on `probe()` — a missing TCB skips L3/L4 (does not fail CI); a NO-GO gate fails. |
