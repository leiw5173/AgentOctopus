...
pnpm test             # run all tests across all workspaces (1,321 cases on macOS: 1,224 pass, 97 platform-gated skip; 148 in the security suite)
...
| `sandbox.defaultBackend` | `'auto'` (default) \| `'docker'` \| `'os'` \| `'windows'`. Restricted OS requires `'os'` plus a restricted floor; restricted Windows requires `'windows'` plus a restricted floor. |
...
**Runtime profiles.** A trusted `runtimeProfiles` entry selects per-backend identity blocks: `dockerImage` for Docker, `osRuntime` for the Linux OS backend, `darwinRuntime` for the Darwin restricted OS backend, and `windowsRuntime` for the Windows restricted backend — `windowsRuntime: { manifestPath, nodePath, bootstrapPath }`, the manifest-verified trusted closure (Node exe + `bootstrap.cjs` + vendored undici) the `WinSandboxBackend` launches skills under. `.strict()` rejects unknown nested fields; `assertRuntimeProfileMatchesBackend` (`sandbox-runner.ts`) fail-fasts a `kind:'windows'` backend without `windowsRuntime` before any topology or proxy launch.
...
**CI runner classes** (security matrix, `packages/sandbox/tests/security/` + the native Windows suite in `packages/sandbox/tests/windows/`):
...
| Windows restricted | `windows-restricted` (`windows-latest`) | The native Windows backend (`WinSandboxBackend`) on a real Windows host: builds the helper + companion gate service + trusted `windowsRuntime` closure via `scripts/build-win-helper.mjs`, installs + starts the `OctopusSandboxGate` service in the runner's elevated context (hosted `windows-latest` steps run as a local admin), then runs the `tests/windows` behavioral suite (helper sid/probe/run/teardown, gate-svc RPC, win-backend lifecycle). Asserts the backend reports exactly `restricted` — never `full`. | **Hosted RESTRICTED lane that must always succeed** — no fork-guard skip and no environment-specific qualification skip (unlike privileged-linux / vm-lane). Zero-skip gate: `assert-no-skipped-tests.mjs` fails the job on ANY skipped test, so a C compile error, an artifact gap, or a failed service install fails the lane VISIBLY. The security-gate requires `windows-restricted = success`; any other outcome (failure, cancellation, unexpected skip) is fail-closed. |
| VM (libkrun) | `vm-hvf-probe` + `vm-lane` (`[self-hosted,macOS,arm64,physical]`) | Real libkrun G1/G2 boot qualification and L3/L4 escape matrix, then an Ed25519-signed release manifest and qualified darwin-arm64 TCB artifact. The linux-arm64 rootfs comes from `produce-linux-artifacts`; macOS never substitutes linux-x64. | Fork PRs skip BOTH physical self-hosted jobs. Same-repo PRs, push, and release calls require a successful entitlement-signed HVF probe and a real zero-skip VM lane; codesign or `hv_vm_create` failure is fatal, never reported as `unavailable`. |

**Security-gate skip policy.** `security-gate` directly requires `produce-linux-artifacts` success on every event. Fork PRs may skip the privileged Linux and both physical macOS VM jobs (trust boundary); same-repo PRs and release runs require the Linux lane, successful HVF probe, and VM lane. Dependency-induced skips are failures. `tests/security/{security-gate-workflow,hvf-probe-workflow}.test.mjs` execute the workflow shell paths and cover these exceptions. The pre-checkout Linux staging step creates the exact exported `/var/tmp/oct-tmp` path and fails if it cannot. Direct master-push security runs and Release Preflight's reusable invocation use separate concurrency groups keyed by `github.workflow`, so neither cancels the other.
...
| `packages/sandbox` | `backend.ts`, `docker/`, `os/`, `vm/`, `windows/`, `proxy/`, `snapshot.ts`, `policy.ts`, `secrets.ts`, `schema.ts` | Leaf isolation package (imports nothing from `@agentoctopus/*` or native). `selectBackend` + `NoFullBackendError`, `DockerBackend`, `OsSandboxBackend`, `VmSandboxBackend`, `WinSandboxBackend` (`windows/win-backend.ts` — restricted-only, opt-in via `defaultBackend:'windows'` + `minIsolationLevel:'restricted'`; orchestrates a native helper exe for Job Object + restricted-token launch/teardown (a `CreateRestrictedToken`-hardened Low-Integrity token; the LPAC/AppContainer token is NOT used on the node path — it is the confirmed necessary trigger of the Node launch crash); the child launches a session-private node.exe copy (the Low-Integrity token cannot open the host toolchain node.exe — a path-based denial, so the copy is staged into the sessionDir and is also the WFP APP_ID key) and the privileged `OctopusSandboxGate` companion service `windows/service/gate-svc.c` for the persistent WFP egress allowlist (scoped to the sandbox `node.exe` application ID) over its two-op install-gate/remove-gate named-pipe RPC; trusted runtime closure verified via `windows/runtime-manifest.ts`), egress proxy (`egress-proxy.ts`, CA, policy engine, DNS, headers, secret channel), immutable snapshot + digest identity, requested∩granted policy. Native VM helpers live in `packages/sandbox-vm-native` (dynamic-imported only from `core`). |
