...
- [x] **Trusted runtime supply chain** — `windowsRuntime: { manifestPath, nodePath, bootstrapPath }` on `ResolvedRuntimeProfile` (strict schema); `verifyWindowsRuntimeManifest` re-verifies the Node exe + `bootstrap.cjs` + vendored undici closure with strict digest/size/mode discipline; `assertRuntimeProfileMatchesBackend` fail-fasts a `windows` backend without `windowsRuntime` before any topology/proxy launch.
