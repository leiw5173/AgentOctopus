...
**Sandbox security suite count:** 139 Vitest cases across 15 files (82 pass, 57 platform-gated skip on a macOS dev host, verified 2026-09-22). The six `security-gate-workflow.test.mjs` cases execute the actual gate shell script against producer failure and authorized/unauthorized Linux/VM skips. A local skip is not CI coverage; the hosted Docker, privileged Linux, Windows, and VM lanes own their real isolation claims.
...
# Sandbox security suite (139 cases; platform-gated skips locally):
