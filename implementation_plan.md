...
**Sandbox security suite count:** 148 Vitest cases across 16 files (91 pass, 57 platform-gated skip on a macOS dev host, verified 2026-09-22). The 15 `security-gate-workflow.test.mjs` and `hvf-probe-workflow.test.mjs` cases exercise the gate and HVF-probe shell steps, fork trust boundaries, staging setup, and master-push concurrency. A local skip is not CI coverage; the hosted Docker, privileged Linux, Windows, and VM lanes own their real isolation claims.

### Phase W — Windows Restricted Sandbox Backend ✅ Complete (Tasks 1–15)

Native Windows isolation for hosts without WSL2/Hyper-V/Docker Desktop: the `WinSandboxBackend` (`packages/sandbox/src/windows/win-backend.ts`) delivers honest **`restricted`** isolation — never `full`, never a VM — by layering four user-mode Windows primitives (Job Object limits + `KILL_ON_JOB_CLOSE`, LPAC capability lockdown at Low Integrity Level with `ALL APPLICATION PACKAGES` opted out, restricted token, and a persistent WFP egress allowlist that permits only the loopback proxy for the skill's package SID). Governing spec: `docs/specs/2026-08-07-windows-sandbox-backend-design.md`. Selection is opt-in only — `defaultBackend:'windows'` + `minIsolationLevel:'restricted'`, byte-for-byte the existing restricted-OS contract (`backend.ts` `explicitRestrictedWindows`); `auto` never picks it.

- [x] **Backend kind + selection gate** — `'windows'` added to `BackendKind` and the `defaultBackend` Zod enum; restricted Windows candidates excluded unless the explicit opt-in pair is set.
- [x] **Trusted runtime supply chain** — `windowsRuntime: { manifestPath, nodePath, bootstrapPath }` on `ResolvedRuntimeProfile` (strict schema); `verifyWindowsRuntimeManifest` re-verifies the Node exe + `bootstrap.cjs` + vendored undici closure with strict digest + size checks per entry (sha256 and byte size — no mode check, unlike the Darwin manifest, since NTFS has no executable-mode bit); `assertRuntimeProfileMatchesBackend` fail-fasts a `windows` backend without `windowsRuntime` before any topology/proxy launch.
- [x] **Native helper (unprivileged)** — `windows/helper/` C exe (probe / sid / grant-acl / run / teardown): race-free suspended→assign→resume Job launch, loopback capability SID derivation (`S-1-15-3-*`), LPAC profile create/delete, READ-only DACL grant on the per-session copy, named-Job cross-process teardown, stdio relay with verbatim exit-code propagation.
- [x] **Privileged companion service** — `OctopusSandboxGate` (`windows/service/gate-svc.c`, LocalSystem, auto-start, one-time elevated install) owns all WFP writes over a strictly-ACL'd named-pipe RPC (`\\.\pipe\octopus-sandbox-gate`) with exactly two ops: `install-gate` (persistent provider + sublayer + 4 filters scoped by `FWPM_CONDITION_ALE_PACKAGE_ID` — permit `TCP 127.0.0.1:<proxyPort>` / optional `[::1]`, block all else incl. V6/UDP/non-proxy loopback; records a session lease) and `remove-gate` (service-side verification: resolves the lease, opens the named Job itself, refuses deletion unless the Job is confirmed dead/empty and the SID + filter keys match). Persistent filters fail closed on crash; startup sweep reclaims stale filters for dead sessions.
- [x] **Per-session snapshot delivery** — stage-copy + byte-for-byte re-verify against `expectedSnapshotDigest` (TOCTOU guard), LPAC SID READ grant, wholesale delete on cleanup; `guestSkillRoot`/`guestCaBundlePath` are the staged-copy paths (per-backend canonical assertion).
- [x] **Cleanup ordering + memoization** — Job terminated and confirmed empty BEFORE WFP filter removal; unconfirmed-dead ⇒ filters kept + `ContainmentCleanupError` (memoized first outcome, identical contract to DockerBackend); post-death filter-removal failure is soft fail-closed residue.
- [x] **CI lane** — `windows-restricted` (`windows-latest`) in `sandbox-security.yml`: builds helper + service + runtime via `scripts/build-win-helper.mjs`, installs + starts the service in the runner's elevated context, runs `tests/windows/` with a zero-skip assertion; asserts the backend reports exactly `restricted`. Security-gate requires `windows-restricted = success` — no fork-guard or environment-specific skip. **Caveat (lane-vs-local delta):** the lane is committed and fail-closed but awaits its first run on a real Windows runner — the native C from Tasks 6-9/11 is authored correct-by-construction on macOS and has never been compiled on the dev host, so the workflow's own comment describes this lane as the code's "first authoritative validation on a real Windows host" (mirroring how Phase S discloses lane-vs-local deltas).
- [x] **Fail-closed availability** — no unprivileged degraded mode: absent/unresponsive service, an unverifiable runtime-closure manifest, a failed helper self-test (`helper probe`), or a failed LPAC launch ⇒ `probe()` returns `false` and the run fail-closes per `minIsolationLevel`. (Probe verifies the runtime closure only; the helper/service exes carry build-time manifests that probe does not re-verify.)
- [x] **Docs sync** — README platform matrix, CLAUDE.md architecture + runner-class table, `docs/core-concepts/sandbox.md` (Windows restricted section + trusted runtime identity), `docs/deployment/security.md` (companion service + honest scope), `TEST_INSTRUCTIONS.md` Phase W rows.
...
# Sandbox security suite (148 cases; platform-gated skips locally):
pnpm --filter @agentoctopus/sandbox exec vitest run tests/security
```

Per-package Vitest cases from `pnpm test` on a macOS dev host (expanded `it.each` cases; platform-gated skips included):

| Package | Test files | Cases | Pass | Skip |
|---|---:|---:|---:|---:|
| `packages/skills` | 21 | 147 | 147 | 0 |
| `packages/registry` | 7 | 49 | 49 | 0 |
| `packages/adapters` | 8 | 31 | 31 | 0 |
| `packages/core` | 29 | 296 | 293 | 3 |
| `packages/sandbox` | 70 | 583 | 501 | 82 |
| `packages/sandbox-vm-native` | 12 | 118 | 106 | 12 |
| `packages/gateway` | 4 | 30 | 30 | 0 |
| `apps/cli` | 6 | 57 | 57 | 0 |
| `apps/web` | 3 | 10 | 10 | 0 |
| **Total** | **160** | **1,321** | **1,224** | **97** |

The privileged Linux and Windows behavioral lanes require their CI runners; macOS skips are not a substitute for those checks.
