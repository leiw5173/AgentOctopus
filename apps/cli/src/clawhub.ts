...
export async function fetchSkillMeta(slug: string, registryUrl?: string): Promise<ClaWHubSkillMeta> {
  ...
  const res = await fetch(`${base}/api/v1/skills/${encodeURIComponent(slug)}`);
  if (!res.ok) {
...
export async function searchSkills(query: string, registryUrl?: string): Promise<ClaWHubSearchResult[]> {
  ...
  return Array.isArray(data) ? data : (data as { results: ClaWHubSearchResult[] }).results || [];
}
 ...
 * Handles the common PKZIP format used by ClaWHub.
 */
