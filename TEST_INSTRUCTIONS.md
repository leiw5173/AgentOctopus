...
| S19 | VM post-probe TOCTOU hardening | `tests/engine.test.ts` (sandbox-vm-native — "post-probe TOCTOU hardening (R4)") | hosted (unit) | `probe()` is the ONLY reader/verifier of `gate-manifest.json` (signature-bound) and the TCB manifest (`verifyVmTcb` returns the exact manifest it verified — no double-read window); the verified state is cached per engine. `resolveRootfs()`/`assertRootfsQualified()`/`start()` fail closed without it and never re-read the gate file — a post-probe swap with a self-consistent unsigned gate is invisible (attacker ref stays unqualified, verified ref still resolves). Prepare boundary (`resolveRootfs`) re-verifies all four TCB artifacts (digest+symlink+mode — covers the image builder consumed next); launch boundary (`start`) re-verifies helper/libkrun/libkrunfw, re-hashes the rootfs vs its ref, and re-checks cached-gate membership. Regression: post-probe swaps of gate/helper/libkrun/image-builder/rootfs each fail closed. |
...
| S19 | VM post-probe TOCTOU hardening (cached gate, boundary re-verify) | ✅ (unit) |
