...
| W6 | Network allowlist — WFP permit-proxy / block-else | `tests/windows/gate-svc.test.ts` (install path) + `tests/windows/win-backend.test.ts` (prepare passes the session package SID + proxy endpoint to installGate) | **Windows host with `OctopusSandboxGate` installed + running** (`itSvc`; skips elsewhere) | Current assertions: `install-gate` returns `ok:true` with a non-empty `filterKeys` array, and the win-backend prepare path hands the service the session package SID + proxy host/port (+ `proxyV6Loopback` when the proxy dual-binds). The full §4c rule-set assertion (provider/sublayer/weights, V4/V6 layers, TCP-only, single permitted endpoint — spec Acceptance #8) is NOT yet implemented in any test; it is the intended lane-side assertion to add on the `windows-restricted` lane, not a current one. |
